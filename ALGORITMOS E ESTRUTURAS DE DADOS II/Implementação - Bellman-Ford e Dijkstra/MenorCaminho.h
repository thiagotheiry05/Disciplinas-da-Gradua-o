...
public:
    ...
    **/
    int* bellmanFord(string rotuloVOrigem) {
        int posicao = obterIndiceVertice(rotuloVOrigem);
    ...
    **/
    int* dijkstra(string rotuloVOrigem) {
        ...
        distancia[origem] = 0;
        priority.push({ 0, origem });

        while (!priority.empty()){
            int vAtual = priority.top().second;
            priority.pop();
            if (visitados[vAtual]) {
                continue;
            }
            ...
            for (pair<int, int> a : this->arestas[vAtual]){
                int destino = a.first;
                int peso = a.second;
                if (distancia[vAtual] + peso < distancia[destino]){
                    distancia[destino] = distancia[vAtual] + peso;
                    priority.push({ -distancia[destino], destino });
                }
            }
        }
        return distancia;
